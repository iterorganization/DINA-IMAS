 ...
 *
 */
void contr_vert_vs3_Outputs_wrapper(const real_T *u0,
			real_T *y0,
			const real_T *xD,
...
if(K_hl==1){
if(kl==0){
	printf("4 +++cont_vert_vs3.flat \n");
f=fopen("controllers/contr_vert_vs3.flat","r");
fscanf(f,"%d ",&N_st);
for(i=0; i<N_st+N_out; i++) for(j=0;j<N_st+N_in; j++) fscanf(f,"%lf",y[i]+j);
fclose(f);
	printf("ebd of reading cont_vert_vs3.flat \n");
/*mexPrintf(" i %d\n",i);*/
kl=1;}
...
if(K_hl>1){
if(kl2==0){
f=fopen("controllers/contr_vert_vs3_hl.flat","r");
fscanf(f,"%d ",&N_st2);
for(i=0; i<N_st2+N_out; i++) for(j=0;j<N_st2+N_in; j++) fscanf(f,"%lf",y2[i]+j);
fclose(f);
	printf("end of reading contr_vert_vs3_hl.flat \n");
...
/*
  ...
  *
  */
void contr_vert_vs3_Update_wrapper(const real_T *u0,
			const real_T *y0,
			real_T *xD,
			const real_T  *n_in,  const int_T  p_width0,
			const real_T  *n_out,  const int_T  p_width1,
...
for(i=0; i<N_st+N_out; i++) for(j=0;j<N_st+N_in; j++) fscanf(f,"%lf",y[i]+j);
fclose(f);
	printf("end of reading contr_vert_vs3.flat \n");
